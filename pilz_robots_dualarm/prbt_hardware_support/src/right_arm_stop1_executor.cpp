...
RightArm_Stop1Executor::RightArm_Stop1Executor(const TServiceCallFunc& right_arm_hold_func,
                             const TServiceCallFunc& right_arm_unhold_func,
                             const TServiceCallFunc& right_arm_recover_func,
                             const TServiceCallFunc& right_arm_halt_func)
{
  state_machine_ = std::unique_ptr<StoStateMachine>(
        new StoStateMachine(right_arm_recover_func, right_arm_halt_func, right_arm_hold_func, right_arm_unhold_func) );

  state_machine_->start();

  worker_thread_ = std::thread(&RightArm_Stop1Executor::workerThreadFun, this);
}

RightArm_Stop1Executor::~RightArm_Stop1Executor()
{
...
void RightArm_Stop1Executor::updateSto(const bool sto)
{
...
bool RightArm_Stop1Executor::updateStoCallback(std_srvs::SetBool::Request &req,
...
void RightArm_Stop1Executor::workerThreadFun()
{
