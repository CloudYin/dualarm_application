...
LeftArm_Stop1Executor::LeftArm_Stop1Executor(const TServiceCallFunc& left_arm_hold_func,
                             const TServiceCallFunc& left_arm_unhold_func,
                             const TServiceCallFunc& left_arm_recover_func,
                             const TServiceCallFunc& left_arm_halt_func)
{
  state_machine_ = std::unique_ptr<StoStateMachine>(
        new StoStateMachine(left_arm_recover_func, left_arm_halt_func, left_arm_hold_func, left_arm_unhold_func) );

  state_machine_->start();

  worker_thread_ = std::thread(&LeftArm_Stop1Executor::workerThreadFun, this);
}

LeftArm_Stop1Executor::~LeftArm_Stop1Executor()
{
...
void LeftArm_Stop1Executor::updateSto(const bool sto)
{
...
bool LeftArm_Stop1Executor::updateStoCallback(std_srvs::SetBool::Request &req,
...
void LeftArm_Stop1Executor::workerThreadFun()
{
